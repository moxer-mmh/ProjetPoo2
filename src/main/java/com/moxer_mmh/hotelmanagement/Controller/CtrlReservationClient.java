...
public class CtrlReservationClient {
	...
	public static void actionAnnulResrvation(JButton btnAnnuler, String nom, String prenom,
		...
		btnAnnuler.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				 ...
				 * String[] Datedebut =
				 * tableResrvClient.getValueAt(tableResrvClient.getSelectedRow(),
				 * 3).toString().split("/");
				 * Client.annulReservationOld(nom, prenom, model,selectedReservationId,
				...
				if (selectedRow != -1) {
					...
					JOptionPane.showMessageDialog(null, "Réservation annulée avec succès", "Confirmation",
	...
	public static void actionModifyResrvation(JButton btnModifier, String nom, String prenom,
			...
			JComboBox<String> startday, JComboBox<String> startmonth, JComboBox<String> startyear,
			JComboBox<String> endday, JComboBox<String> endmonth, JComboBox<String> endyear, JButton btnEnregistrer,
			JButton btnModif2, JComboBox<TypeChambre> etatChambreC) {
	...
	public static void actionModify2Resrvation(JButton btnModif2, String nom, String prenom,
			...
			JButton btnReservation, JPanel pReservation, JPanel panel_1, JPanel pMesReserv,
			JComboBox<String> startday, JComboBox<String> startmonth, JComboBox<String> startyear,
			JComboBox<String> endday, JComboBox<String> endmonth, JComboBox<String> endyear) {
		...
		btnModif2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String startDate = startday.getSelectedItem().toString() + "/" + startmonth.getSelectedItem().toString()
						+ "/" + startyear.getSelectedItem().toString();
				String endDate = endday.getSelectedItem().toString() + "/" + endmonth.getSelectedItem().toString() + "/"
						+ endyear.getSelectedItem().toString();

				try {
					Client.validateDates(startDate, endDate);
				...
				reservation.setDateDebut(startday.getSelectedItem().toString() + "/"
						+ startmonth.getSelectedItem().toString() + "/" + startyear.getSelectedItem().toString());
				reservation.setDateFin(endday.getSelectedItem().toString() + "/" + endmonth.getSelectedItem().toString()
						+ "/" + endyear.getSelectedItem().toString());

				JOptionPane.showMessageDialog(null, "Réservation modifiée avec succès", "Confirmation",
						JOptionPane.INFORMATION_MESSAGE);
