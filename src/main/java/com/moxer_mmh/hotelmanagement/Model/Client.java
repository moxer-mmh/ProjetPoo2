...
public class Client {
	...
	public void setId(int id) {
	 ...
	 public static void makeReservation(String startday, String startmonth, String
			  startyear,String endday, String endmonth, String endyear, String etatChambreC, String nom, String prenom) {
			  ...
			  "Erreur", JOptionPane.ERROR_MESSAGE); } else {
			  
			  for (Chambre rowList : Chambre.chambres.values()) { rowList.getNumero();
			  rowList.getType(); rowList.getEtatChambre(); }
			  
			  boolean roomFound = false; for (Map.Entry<Integer, Chambre> entry :
			  Chambre.chambres.entrySet()) { Chambre chambre = entry.getValue(); if
			  (chambre.getType().toString() == etatChambreC &&  chambre.getEtatChambre() == EtatChambres.LIBRE) { roomFound = true;
			  
			  String startDate = startday+ "/" + startmonth + "/" + startyear; 
			  String endDate =  endday + "/" + endmonth+ "/" +  endyear;
			  
			  try { validateDates(startDate, endDate); } catch (InvalidDateException e1) {
			  ...
			  chambre.setEtatChambre(EtatChambres.EN_ATTENTE); System.out.println(rowRes +
			  "Réservation effectuée avec succès !"); break; } } if (!roomFound) {
...
public static void annulReservationOld(String nom, String prenom,
        DefaultTableModel model,int selectedReservationId, String[] Datedebut,EtatReservation selectedReservationEtat) {
            ...
            for (Map.Entry<Integer, Reservation> entry : Reservation.getReservations().entrySet()) {
                ...
                if (client.getNom().equals(nom) && client.getPrenom().equals(prenom)) {
                    ...
                    if (reservation.getEtat() == EtatReservation.EN_ATTENTE ) {
                    		//&& (Integer.parseInt(Datedebut[2]) > Integer.parseInt(Date.getTodayyear()) || (Integer.parseInt(Datedebut[2]) == (Integer.parseInt(Date.getTodayyear()) && (Integer.parseInt(Datedebut[1]) > (Integer.parseInt(Date.getTodaymonth() )|| ((Integer.parseInt(Datedebut[1]) == (Integer.parseInt(Date.getTodaymonth()) && (Integer.parseInt(Datedebut[0]) > (Integer.parseInt( Date.getTodayday())){
