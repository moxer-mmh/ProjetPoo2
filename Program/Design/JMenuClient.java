...
public class JMenuClient extends JFrame {
	...
	public JMenuClient(String nom, String prenom) {
		...
		reservClient.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				remplirTableReservations(Administrateur.reservations, nom, prenom);
				pReservation.setVisible(false);
	...
	public void addReservation(String startDate, String endDate) {
		...
		if (Administrateur.chambres == null || Administrateur.chambres.isEmpty()) {
			...
			for (Map.Entry<Integer, Chambres> entry : Administrateur.chambres.entrySet()) {
				...
				if (chambre.getType() == etatChambreC.getSelectedItem()
					...
					Administrateur.reservations.put(row, reservation);
	...
	private void annulerReservation(String nomClient, String prenomClient) {
		...
		for (Map.Entry<Integer, Reservations> entry : Administrateur.reservations.entrySet()) {
			...
			if (client.getNom().equals(nomClient) && client.getPrenom().equals(prenomClient)
					&& reservation.getId() == selectedReservationId) {
	...
	private void remplirTableReservations(Map<Integer, Reservations> reservations, String nomClient,
			String prenomClient) {
		...
		tableClient.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (row >= 0) {
					...
					selectedReservationEtat = (EtatReservation) tableClient.getValueAt(row, 5); // Récupérer l'état de
																								// la réservation
