...
public class Client {
	...
	public static void annulReservation(String nom, String prenom,
		...
		for (Map.Entry<Integer, Reservation> entry : Reservation.getReservations().entrySet()) {
			...
			if (client.getNom().equals(nom) && client.getPrenom().equals(prenom)) {
				...
				if (reservation.getEtat() == EtatReservation.EN_ATTENTE
						&& (Integer.parseInt(Datedebut[2]) > Date.getTodayyear()
								|| (Integer.parseInt(Datedebut[2]) == Date.getTodayyear()
										&& (Integer.parseInt(Datedebut[1]) > Date.getTodaymonth()
												|| ((Integer.parseInt(Datedebut[1]) == Date.getTodaymonth())
														&& (Integer.parseInt(Datedebut[0]) > Date.getTodayday())))))) {
